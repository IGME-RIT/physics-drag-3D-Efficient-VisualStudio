...
and the relative velocity between the medium and the object. This algorithm is inaccurate- but efficient.
The cross sectional area is calculated as the bounding rectangle of the object projected onto the subspace
...
///
//Calculates and applied the force due to form drag on an object. This is done slightly inaccurately, but efficiently.
//This algorithm computes drag by assuming the cross-sectional area of the  object to be rectangular.
//
//Parameters:
...
void CalculateDrag(float fluidDensity, glm::vec3 fluidVelocity, struct RigidBody &body, const struct ConvexHull &collider)
{
	...
	//Step 2: Project the convex hull onto these axes and determine the minimum and maximum scalar projection values
	float minX, maxX, minY, maxY, dot;
	glm::vec3 currentVertex = collider.rotation * collider.scale * collider.vertices[0];
	minX = maxX = glm::dot(x, currentVertex);
	minY = maxY = glm::dot(y, currentVertex);
	int size = collider.vertices.size();
	for(int i = 0; i < size; i++)
	{
		currentVertex = collider.rotation * collider.scale * collider.vertices[i];
		dot = glm::dot(x, currentVertex);
		if(dot < minX) minX = dot;
		if(dot > maxX) maxX = dot;

		dot = glm::dot(y, currentVertex);
		if(dot < minY) minY = dot;
		if(dot > maxY) maxY = dot;
	}

	//Calculate the dimension of the hull along these axes
	...
	//This is where this algorithm becomes inaccurate-- in order to maintain efficiency, we will simply calculate the area of the rectangular plane with the dimensions dimX and dimY.
	float area = dimX * dimY;
...
void main()
{
	...
	// Create a window
	window = glfwCreateWindow(800, 800, "Drag (3D - efficient)", nullptr, nullptr);
	...
	tetrahedron->scale = glm::scale(tetrahedron->scale, glm::vec3(0.1f));
